...
def extractText():
    if "user" in session:
        ...
        response = requests.post(api_endpoint, files={'image': image_bytes}, data=payload)
    
        # Send the POST request to the API
        # response = requests.post(api_endpoint, files={'image': image}, data=payload)
        response_data = response.json()

    
        # Extract the text from the response
        if response_data['IsErroredOnProcessing']:
            return jsonify({'error': 'Error occurred during text extraction.'}), 400
        else:
...
def login(mainstr):
    if mainstr not in ["login", "signup", "google","whatsapp"]:
        abort(404)
    if 'user' in session:
...
def configuration():
    if 'user' in session:
        ...
        if request.method=="POST":
            ...
            mobile=request.form.get("mobile")
            imgWidth=request.form.get("imgWidth") or " "
            imgHeight=request.form.get("imgHeight") or " "
            imgFormat=request.form.get("imgFormat") or " "
            watermarkText=request.form.get("watermarkText") or " "
            watermarkPositon=request.form.get("watermarkPositon") or " "
            watermarkFontSize=request.form.get("watermarkFontSize") or " "
            aboutMe=request.form.get("aboutMe") or " "
            ...
            config=Configurations.query.filter_by(userId=userId).first()
            if config is None:
                new_config=Configurations(id=id,userId=userId,imgWidth=imgWidth,imgHeight=imgHeight,imgFormat=imgFormat,watermarkText=watermarkText,watermarkPositon=watermarkPositon,watermarkFontSize=watermarkFontSize,aboutMe=aboutMe)
                ...
                config.watermarkPositon=watermarkPositon
                config.watermarkFontSize=watermarkFontSize
                config.aboutMe=aboutMe
...
# creates avatar
@app.route("/avatar",methods=['GET','POST',"DELETE"])   
def create_avatar():
    if 'user' in session:
        
        userId=session['user']
        
        url="static/savedImg/"+str(userId)+"/"+"image_775dc38b58374098bf78ac8ced515b60.png"
        size=500  
        # Open the image
        ...
        masked_image = Image.new('RGBA', (size, size))
        masked_image.paste(image, (0, 0), mask)

        # Save the avatar image
        avatar_path = 'avatar.png'
        masked_image.save(avatar_path)
...
def inject_user():
    ...
    return dict(params=params,user=user,config=config)  