...
class Configurations(db.Model):
    ...
    id=db.Column(db.String(50),nullable=False,primary_key=True,unique=True)
    userId=db.Column(db.String(50),db.ForeignKey('users.id'),nullable=False,unique=True)
    imgWidth=db.Column(db.String(5),nullable=True)
    imgHeight=db.Column(db.String(5),nullable=True)
    imgFormat=db.Column(db.String(15),nullable=True)
    watermarkText=db.Column(db.String(250),nullable=True)
    watermarkPositon=db.Column(db.String(25),nullable=True)
    watermarkFontSize=db.Column(db.String(5),nullable=True)
    aboutMe=db.Column(db.String(500),nullable=True)
    date=db.Column(db.DateTime, default=datetime.now(),nullable=True)
    user = db.relationship('Users', backref='configurations')
